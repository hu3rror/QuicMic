...
14. **Filter fake-ip TUN addresses from LAN IP detection**: `detect_lan_ip` must trust the default-route pick only when it is a usable LAN address (`is_unusable_lan_addr` rejects `0.0.0.0/8`, loopback, link-local, multicast/reserved `240/4`, and the RFC 2544 benchmark range `198.18.0.0/15` — the de-facto fake-ip default for mihomo/Clash, sing-box, Surge); otherwise fall back to the ranked interface scan (`pick_lan_ip` + `lan_addr_rank`: RFC 1918 (`v4.is_private()`) > CGNAT `100.64/10` and IPv6 > public IPv4, IPv4 over IPv6 on rank ties; same-rank same-family ties prefer physical adapters — virtual ones (`is_virtual_adapter`: `vEthernet`/`wsl`/`docker`/`vmnet`/`vbox`/`tun`/… name patterns) lose to the real NIC). `--ip` remains the manual override. (ADR-0004)
