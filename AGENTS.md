...
- **Proxy-TUN fake-ip filtering (`detect_lan_ip` in `main.rs`):** `local_ip_address::local_ip()` returns the first IPv4 unicast address among the adapters that own a *default route*, which is wrong when a proxy TUN (mihomo/Clash with fake-ip, e.g. `device: sekai` holding `198.18.0.1`) owns a default route with a lower metric: the TUN's fake address wins even though nothing on the LAN can route to it. So the default-route pick is trusted only when it is a **usable LAN address** (`is_unusable_lan_addr` rejects `0.0.0.0/8`, loopback, link-local, multicast/reserved `240/4`, and the RFC 2544 benchmark range `198.18.0.0/15` — the mihomo/Clash fake-ip default); otherwise `list_afinet_netifas()` is scanned and the best-ranked candidate is picked (`pick_lan_ip` + `lan_addr_rank`: RFC 1918 (2) > CGNAT `100.64/10` and IPv6 (1) > public IPv4 (0), IPv4 over IPv6 on rank ties). Same-rank same-family ties (e.g. Wi-Fi `192.168.x` vs a Hyper-V/VPN `172.x`) resolve by OS enumeration order (`max_by_key`'s last maximum) — reproducible but not semantic; `--ip` is the manual override. The pure functions are unit-tested.
