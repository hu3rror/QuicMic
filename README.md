...
- **VPN / proxy TUN adapters are filtered out.** QuicMic skips the fake address that proxy TUN adapters (mihomo/Clash with fake-ip and similar, which use the `198.18.0.0/15` range) put on the default route, and picks a real LAN address instead. If the auto-detected address is still wrong in an unusual network layout, override it with `--ip <your-LAN-IP>`.
