...
- **Installable entry** (ADR-0018): the page is installable from a phone home screen — `web/manifest.webmanifest` (W3C-standard fields, `start_url: "/"`, `display: "standalone"`, 192/512 icons), `web/icons/icon-{180,192,512}.png`, and a **minimal service worker** (`web/sw.js`: register + empty fetch handler, no caching) whose only purpose is to satisfy Chromium installability criteria for a future trusted-CA origin. The home-screen window opens `/` with no hash, so it routes through the ADR-0017 resume path: a paired device lands on the main screen with zero PIN interaction. The strict CSP gains `worker-src 'self'`. Chromium's automatic install prompt remains unavailable under the self-signed certificate (not a secure context) — manual "Add to Home Screen" is the path.
- **In-app QR scan** (ADR-0019): the pairing card's Scan button opens a full-screen viewfinder that decodes the same Pairing QR with the phone's own camera — aimed at the storage-isolated home-screen container (ADR-0018), whose first pairing previously needed a separate camera app or manual typing. The QR content is unchanged (`URL+#PIN`); the decoder extracts the trailing 6-digit hash and feeds the existing typed-PIN pair path, so scanning is exactly equivalent to typing: no hash mutation, no token clear, no history change, one pair attempt per decode (never auto-retried, so a bad scan cannot trip the pairing throttle, ADR-0012). Camera denial / cancel / page-hide degrade to a toast plus manual entry — the camera never gates pairing. The pure logic (text parsing, vendored jsQR wrapper, error mapping) lives in `web/qr.js` (Node unit tests incl. a committed PPM fixture decoded end-to-end through the vendored jsQR); `app.js` keeps the getUserMedia / viewfinder / pairing glue. iOS standalone PWA camera access is a known risk, characterized on-device.
