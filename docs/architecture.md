...
- **Client storage**: `localStorage` is the source of truth for settings; pushed to the server on pairing / token renewal / reconnection; server values adopted only on a true first run (ADR-0008).
...
- TLS: self-signed ECDSA P-256 cert generated in-memory; client pins the SHA-256 of the DER via `serverCertificateHashes` fetched from `/api/info`; 14-day lifetime; `--dump-certs` for debugging (ADR-0006).
